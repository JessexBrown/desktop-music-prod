...
- Project package model that writes a human-readable `manifest.json` plus
  ...
  project and AppSession invalid loop-region schema failures, project invalid
  loop-region value failures, manifest-directory and symlinked-manifest loads,
  leaving isolated app settings untouched.
...
- Project load rejects package paths occupied by files, direct package
  ...
  missing manifests, manifest directories, and manifest symlinks before parsing
  them; session
  ...
  failures, project invalid loop-region value failures, project and AppSession
  package-path file failures, project and AppSession direct package symlink
  failures, including broken links, project linked/broken package-parent
  failures,
