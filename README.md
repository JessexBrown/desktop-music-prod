...
- Project package model that writes a human-readable `manifest.json` plus
  ...
  project and AppSession unsupported-version failures, project invalid-tracks
  failures, plus manifest-directory and symlinked-manifest loads, leaving
  isolated app settings untouched.
...
- Project load rejects package paths occupied by files, direct package
  ...
  failures, project invalid-tracks failures, project and AppSession
  package-path file
  failures, project and AppSession direct package symlink failures, including
  broken links, project linked/broken package-parent failures,
