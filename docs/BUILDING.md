...
tests, project and AppSession malformed-manifest load failure/app-settings
...
schema and value load failure/app-settings isolation tests, project and
AppSession invalid-devices schema load failure/app-settings isolation tests,
...
tests, AppSession linked/broken package-parent settings-isolation tests, and
...
host-supported,
project and AppSession package-path file load settings-isolation tests,
package-path file load/save rejection tests, package directory creation failure
tests, AppSession package-path file load rejection tests,
...
session-level manifest and linked/broken asset-folder symlink save failure
tests, when host-supported, staged temporary-manifest open/commit failure
tests, Save As retry manifest/asset
...
background Save As linked and broken source/target symlink failure tests when
host-supported, plus app settings load-directory, symlink, broken-symlink,
empty-path, load-linked-parent-symlink, load-broken-parent-symlink,
...
and broken-symlink settings-file plus linked and broken parent load/save
rejection when host-supported.
