...
void appSessionMalformedManifestLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Malformed Manifest Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Malformed Manifest Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-malformed-manifest-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session malformed-manifest settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Malformed Manifest Settings Isolation Baseline");
    session.setTempoBpm(129.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-malformed-manifest-load-settings-isolation-test");
    const auto malformedManifestText = std::string("{ not valid json");
    const auto sentinelPath = package / "sentinel.txt";
    writeManifestText(package, malformedManifestText);
    writeTextFile(sentinelPath, "session malformed manifest settings-isolation sentinel");

    error = "stale session malformed manifest settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects malformed manifest during settings-isolation check");
    expect(error.find("Project manifest is not valid JSON") != std::string::npos,
           "Session malformed-manifest settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session malformed-manifest settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session malformed-manifest settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session malformed-manifest settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == malformedManifestText,
           "Session malformed-manifest settings-isolation load failure preserves malformed manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session malformed-manifest settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session malformed manifest settings-isolation sentinel",
           "Session malformed-manifest settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session malformed-manifest settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session malformed-manifest settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session malformed-manifest settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session malformed-manifest settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session malformed-manifest settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session malformed-manifest settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
...
void appSessionUnsupportedManifestVersionLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Unsupported Version Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Unsupported Version Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-unsupported-version-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session unsupported-version settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Unsupported Version Settings Isolation Baseline");
    session.setTempoBpm(143.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-unsupported-version-load-settings-isolation-test");
    const auto unsupportedManifestText = std::string(R"({
  "manifestVersion": 999,
  "name": "Too New Session Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": []
    ...
    writeManifestText(package, unsupportedManifestText);
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "session unsupported version settings-isolation sentinel");

    error = "stale session unsupported version settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects unsupported manifest version during settings-isolation check");
    expect(error.find("Unsupported project manifest version") != std::string::npos,
           "Session unsupported-version settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session unsupported-version settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session unsupported-version settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session unsupported-version settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Session unsupported-version settings-isolation load failure preserves unsupported manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session unsupported-version settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session unsupported version settings-isolation sentinel",
           "Session unsupported-version settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session unsupported-version settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session unsupported-version settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session unsupported-version settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session unsupported-version settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session unsupported-version settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session unsupported-version settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary session unsupported-version settings-isolation app settings file deleted");
}

void projectInvalidLoopRegionSchemaLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Invalid Loop Region Schema Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Invalid Loop Region Schema Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-invalid-loop-region-schema-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Invalid-loop-region schema settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    const auto package =
        makeTemporaryPackagePath("projectname-invalid-loop-region-schema-load-settings-isolation-test");
    const auto invalidLoopRegionManifestText = std::string(R"({
  "manifestVersion": 1,
  "name": "Bad Loop Region Schema Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": []
    ...
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "invalid loop region schema settings-isolation sentinel");

    error = "stale invalid loop region schema settings-isolation load error";
    auto loaded = projectname::ProjectModel::loadPackage(package, error);
    expect(!loaded.has_value(),
           "Project load rejects invalid loop-region schema during settings-isolation check");
    expect(error.find("Project loop region must be an object") != std::string::npos,
           "Invalid-loop-region schema settings-isolation load failure error is human-readable");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Invalid-loop-region schema settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Invalid-loop-region schema settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Invalid-loop-region schema settings-isolation load failure preserves manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Invalid-loop-region schema settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "invalid loop region schema settings-isolation sentinel",
           "Invalid-loop-region schema settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Invalid-loop-region schema settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Invalid-loop-region schema settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Invalid-loop-region schema settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Invalid-loop-region schema settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Invalid-loop-region schema settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary invalid-loop-region schema settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary invalid-loop-region schema settings-isolation app settings file deleted");
}

void appSessionInvalidLoopRegionSchemaLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Invalid Loop Region Schema Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Invalid Loop Region Schema Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-invalid-loop-region-schema-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session invalid-loop-region schema settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Invalid Loop Region Schema Settings Isolation Baseline");
    session.setTempoBpm(137.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-invalid-loop-region-schema-load-settings-isolation-test");
    const auto invalidLoopRegionManifestText = std::string(R"({
  "manifestVersion": 1,
  "name": "Bad Loop Region Schema Session Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": []
    ...
    writeManifestText(package, invalidLoopRegionManifestText);
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "session invalid loop region schema settings-isolation sentinel");

    error = "stale session invalid loop region schema settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects invalid loop-region schema during settings-isolation check");
    expect(error.find("Project loop region must be an object") != std::string::npos,
           "Session invalid-loop-region schema settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session invalid-loop-region schema settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session invalid-loop-region schema settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session invalid-loop-region schema settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Session invalid-loop-region schema settings-isolation load failure preserves manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session invalid-loop-region schema settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session invalid loop region schema settings-isolation sentinel",
           "Session invalid-loop-region schema settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session invalid-loop-region schema settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session invalid-loop-region schema settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session invalid-loop-region schema settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session invalid-loop-region schema settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session invalid-loop-region schema settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session invalid-loop-region schema settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
...
void appSessionInvalidLoopRegionValueLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Invalid Loop Region Value Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Invalid Loop Region Value Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-invalid-loop-region-value-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session invalid-loop-region value settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Invalid Loop Region Value Settings Isolation Baseline");
    session.setTempoBpm(128.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-invalid-loop-region-value-load-settings-isolation-test");
    const auto invalidLoopRegionManifestText = std::string(R"({
  "manifestVersion": 1,
  "name": "Bad Loop Region Value Session Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": []
    ...
    writeManifestText(package, invalidLoopRegionManifestText);
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "session invalid loop region value settings-isolation sentinel");

    error = "stale session invalid loop region value settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects invalid loop-region value during settings-isolation check");
    expect(error.find("Project loop region is invalid") != std::string::npos,
           "Session invalid-loop-region value settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session invalid-loop-region value settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session invalid-loop-region value settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session invalid-loop-region value settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Session invalid-loop-region value settings-isolation load failure preserves manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session invalid-loop-region value settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session invalid loop region value settings-isolation sentinel",
           "Session invalid-loop-region value settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session invalid-loop-region value settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session invalid-loop-region value settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session invalid-loop-region value settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session invalid-loop-region value settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session invalid-loop-region value settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session invalid-loop-region value settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
...
void appSessionInvalidDevicesSchemaLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Invalid Devices Schema Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Invalid Devices Schema Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-invalid-devices-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session invalid-devices schema settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Invalid Devices Settings Isolation Baseline");
    session.setTempoBpm(104.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-invalid-devices-load-settings-isolation-test");
    const auto invalidDevicesManifestText = std::string(R"({
  "manifestVersion": 1,
  "name": "Bad Devices Session Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": [
    {
      "id": "track-session-invalid-devices",
      "name": "Session Invalid Devices Track",
      "type": "audio",
      "devices": {},
      "clips": []
    }
    ...
    writeManifestText(package, invalidDevicesManifestText);
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "session invalid devices settings-isolation sentinel");

    error = "stale session invalid devices settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects invalid devices schema during settings-isolation check");
    expect(error.find("Track devices must be an array") != std::string::npos,
           "Session invalid-devices schema settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session invalid-devices schema settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session invalid-devices schema settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session invalid-devices schema settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Session invalid-devices schema settings-isolation load failure preserves manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session invalid-devices schema settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session invalid devices settings-isolation sentinel",
           "Session invalid-devices schema settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session invalid-devices schema settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session invalid-devices schema settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session invalid-devices schema settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session invalid-devices schema settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session invalid-devices schema settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session invalid-devices schema settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary session invalid-devices schema settings-isolation app settings file deleted");
}
...
void appSessionInvalidTracksSchemaLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Invalid Tracks Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Invalid Tracks Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-invalid-tracks-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session invalid-tracks settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Invalid Tracks Settings Isolation Baseline");
    session.setTempoBpm(111.0);
    const auto originalProject = session.getProject();

    const auto package =
        makeTemporaryPackagePath("projectname-session-invalid-tracks-load-settings-isolation-test");
    const auto invalidTracksManifestText = std::string(R"({
  "manifestVersion": 1,
  "name": "Bad Tracks Session Settings Isolation",
  "transport": {
    "tempoBpm": 120,
    "timeSignature": { "numerator": 4, "denominator": 4 },
    "positionBeats": 0
  ...
  "tracks": {}
    ...
    writeManifestText(package, invalidTracksManifestText);
    const auto originalManifestText = readTextFile(package / "manifest.json");
    writeTextFile(sentinelPath, "session invalid tracks settings-isolation sentinel");

    error = "stale session invalid tracks settings-isolation load error";
    expect(!session.loadProjectPackage(package, error),
           "Session load rejects invalid tracks schema during settings-isolation check");
    expect(error.find("Project tracks must be an array") != std::string::npos,
           "Session invalid-tracks settings-isolation load failure error is human-readable");
    expect(session.getProject() == originalProject,
           "Session invalid-tracks settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session invalid-tracks settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session invalid-tracks settings-isolation load failure leaves app settings loadable");
    expect(readTextFile(package / "manifest.json") == originalManifestText,
           "Session invalid-tracks settings-isolation load failure preserves manifest");
    expect(!std::filesystem::exists(package / "manifest.json.tmp"),
           "Session invalid-tracks settings-isolation load failure does not create a temporary manifest");
    expect(readTextFile(sentinelPath) == "session invalid tracks settings-isolation sentinel",
           "Session invalid-tracks settings-isolation load failure preserves package contents");
    expect(!std::filesystem::exists(package / "audio"),
           "Session invalid-tracks settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(package / "samples"),
           "Session invalid-tracks settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(package / "presets"),
           "Session invalid-tracks settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(package / "analysis"),
           "Session invalid-tracks settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(package / "backups"),
           "Session invalid-tracks settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove_all(package) > 0,
           "Temporary session invalid-tracks settings-isolation package deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary session invalid-tracks settings-isolation app settings file deleted");
}
...
void projectLoadPackagePathFileFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Project Package Path File Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Project Package Path File Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-load-package-file-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Project package-path file settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    const auto occupiedPackagePath =
        makeTemporaryPackagePath("projectname-load-package-file-settings-isolation-test");
    writeTextFile(occupiedPackagePath, "occupied project package settings-isolation load path");

    error = "stale package path file settings-isolation load error";
    auto loaded = projectname::ProjectModel::loadPackage(occupiedPackagePath, error);
    expect(!loaded.has_value(),
           "Project load rejects a package path file during settings-isolation check");
    expect(error.find("Project package path points to a file") != std::string::npos,
           "Project package-path file settings-isolation load failure error is human-readable");
    expect(error.find("JSON") == std::string::npos,
           "Project package-path file settings-isolation load failure does not parse JSON");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Project package-path file settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Project package-path file settings-isolation load failure leaves app settings loadable");
    expect(std::filesystem::is_regular_file(occupiedPackagePath),
           "Project package-path file settings-isolation load failure leaves occupied path as a file");
    expect(readTextFile(occupiedPackagePath) == "occupied project package settings-isolation load path",
           "Project package-path file settings-isolation load failure preserves occupied file contents");
    expect(!std::filesystem::exists(occupiedPackagePath / "manifest.json"),
           "Project package-path file settings-isolation load failure does not create a manifest below occupied path");
    expect(!std::filesystem::exists(occupiedPackagePath / "manifest.json.tmp"),
           "Project package-path file settings-isolation load failure does not create a temporary manifest below occupied path");
    expect(!std::filesystem::exists(occupiedPackagePath / "audio"),
           "Project package-path file settings-isolation load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "samples"),
           "Project package-path file settings-isolation load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "presets"),
           "Project package-path file settings-isolation load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "analysis"),
           "Project package-path file settings-isolation load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "backups"),
           "Project package-path file settings-isolation load failure does not create a backups asset folder");

    expect(std::filesystem::remove(occupiedPackagePath),
           "Temporary occupied project package settings-isolation load path file deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary project package-path file settings-isolation app settings file deleted");
}

void appSessionLoadPackagePathFileFailureKeepsSessionProject()
{
    projectname::AppSession session;
    session.getProject().setName("Keep Package Path File Session Baseline");
    session.setTempoBpm(141.0);
    const auto originalProject = session.getProject();

    const auto occupiedPackagePath =
        makeTemporaryPackagePath("projectname-session-load-package-file-failure-test");
    writeTextFile(occupiedPackagePath, "occupied session project package load path");

    std::string error = "stale session package path file load error";
    expect(!session.loadProjectPackage(occupiedPackagePath, error),
           "Session load rejects a package path occupied by a file");
    expect(error.find("Project package path points to a file") != std::string::npos,
           "Session package-path file load failure error is human-readable");
    expect(error.find("JSON") == std::string::npos,
           "Session package-path file load failure does not parse JSON");
    expect(session.getProject() == originalProject,
           "Session package-path file load failure keeps current session project");
    expect(std::filesystem::is_regular_file(occupiedPackagePath),
           "Session package-path file load failure leaves occupied path as a file");
    expect(readTextFile(occupiedPackagePath) == "occupied session project package load path",
           "Session package-path file load failure preserves occupied file contents");
    expect(!std::filesystem::exists(occupiedPackagePath / "manifest.json"),
           "Session package-path file load failure does not create a manifest below occupied path");
    expect(!std::filesystem::exists(occupiedPackagePath / "manifest.json.tmp"),
           "Session package-path file load failure does not create a temporary manifest below occupied path");
    expect(!std::filesystem::exists(occupiedPackagePath / "audio"),
           "Session package-path file load failure does not create an audio asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "samples"),
           "Session package-path file load failure does not create a samples asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "presets"),
           "Session package-path file load failure does not create a presets asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "analysis"),
           "Session package-path file load failure does not create an analysis asset folder");
    expect(!std::filesystem::exists(occupiedPackagePath / "backups"),
           "Session package-path file load failure does not create a backups asset folder");
...
void projectBrokenDirectPackageSymlinkLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Broken Direct Package Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Broken Direct Package Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-broken-direct-package-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Broken direct package symlink settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    const auto packageSymlink =
        makeTemporaryPackagePath("projectname-broken-direct-package-load-settings-isolation-link-test");
    const auto missingTargetPackage =
        makeTemporaryPackagePath("projectname-broken-direct-package-load-settings-isolation-target-test");
    ...
    error = "stale broken direct package settings-isolation load error";
    auto loaded = projectname::ProjectModel::loadPackage(packageSymlink, error);
    expect(!loaded.has_value(),
           "Project load rejects a broken direct package symlink during settings-isolation check");
    expect(error.find("Project package path contains a symlink") != std::string::npos
               && error.find(packageSymlink.generic_string()) != std::string::npos,
           "Broken direct package symlink settings-isolation load failure error is human-readable");
    expect(error.find("not found") == std::string::npos,
           "Broken direct package symlink settings-isolation load failure is not reported as a missing manifest");
    expect(error.find("JSON") == std::string::npos,
           "Broken direct package symlink settings-isolation load failure does not parse JSON through the symlink");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Broken direct package symlink settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Broken direct package symlink settings-isolation load failure leaves app settings loadable");
    expect(std::filesystem::is_symlink(std::filesystem::symlink_status(packageSymlink)),
           "Broken direct package symlink settings-isolation load failure leaves the package symlink unchanged");
    expect(!std::filesystem::exists(missingTargetPackage),
           "Broken direct package symlink settings-isolation load failure does not create the missing target");
    expect(!std::filesystem::exists(packageSymlink / "manifest.json.tmp"),
           "Broken direct package symlink settings-isolation load failure does not create a temporary manifest through the link");
    expect(!std::filesystem::exists(missingTargetPackage / "manifest.json.tmp"),
           "Broken direct package symlink settings-isolation load failure does not write target temporary manifest");

    expect(std::filesystem::remove(packageSymlink),
           "Temporary broken direct package symlink settings-isolation symlink deleted");
    std::filesystem::remove_all(missingTargetPackage);
    expect(std::filesystem::remove(settingsPath),
           "Temporary broken direct package symlink settings-isolation app settings file deleted");
}
...
void appSessionLinkedPackageParentSymlinkLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Linked Parent Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Linked Parent Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-linked-package-parent-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session linked package-parent settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Linked Parent Settings Isolation Baseline");
    session.setTempoBpm(137.0);
    const auto originalProject = session.getProject();

    const auto parentSymlink =
        makeTemporaryPackagePath("projectname-session-linked-package-parent-load-settings-isolation-link-test");
    const auto linkedParentTarget =
        makeTemporaryPackagePath("projectname-session-linked-package-parent-load-settings-isolation-target-test");
    const auto linkedTargetPackage = linkedParentTarget / "Nested Session Load.project";
    const auto requestedPackage = parentSymlink / "Nested Session Load.project";
    const auto linkedParentSentinel = linkedParentTarget / "sentinel.txt";
    const auto linkedManifestText =
        R"({"manifestVersion":1,"name":"Session Linked Parent Settings Isolation Manifest Should Not Load","transport":{"tempoBpm":88.0,"timeSignature":{"numerator":5,"denominator":4},"positionBeats":2.0},"tracks":[]})";

    writeManifestText(linkedTargetPackage, linkedManifestText);
    writeTextFile(linkedParentSentinel, "session linked parent settings-isolation sentinel");
    ...
    error = "stale session linked package-parent settings-isolation load error";
    expect(!session.loadProjectPackage(requestedPackage, error),
           "Session load rejects a linked intermediate parent during settings-isolation check");
    expect(error.find("Project package path contains a symlink") != std::string::npos
               && error.find(parentSymlink.generic_string()) != std::string::npos,
           "Session linked package-parent settings-isolation load failure error is human-readable");
    expect(error.find("JSON") == std::string::npos,
           "Session linked package-parent settings-isolation load failure does not parse JSON through the symlink");
    expect(session.getProject() == originalProject,
           "Session linked package-parent settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session linked package-parent settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session linked package-parent settings-isolation load failure leaves app settings loadable");
    expect(std::filesystem::is_symlink(std::filesystem::symlink_status(parentSymlink)),
           "Session linked package-parent settings-isolation load failure leaves the parent symlink unchanged");
    expect(readTextFile(linkedTargetPackage / "manifest.json") == linkedManifestText,
           "Session linked package-parent settings-isolation load failure preserves the linked target manifest");
    expect(readTextFile(linkedParentSentinel) == "session linked parent settings-isolation sentinel",
           "Session linked package-parent settings-isolation load failure preserves the linked target sentinel");
    expect(!std::filesystem::exists(requestedPackage / "manifest.json.tmp"),
           "Session linked package-parent settings-isolation load failure does not create a temporary manifest through the link");
    expect(!std::filesystem::exists(linkedTargetPackage / "manifest.json.tmp"),
           "Session linked package-parent settings-isolation load failure does not write target temporary manifest");

    expect(std::filesystem::remove(parentSymlink),
           "Temporary session linked package-parent settings-isolation symlink deleted");
    expect(std::filesystem::remove_all(linkedParentTarget) > 0,
           "Temporary session linked package-parent settings-isolation target deleted");
    expect(std::filesystem::remove(settingsPath),
           "Temporary session linked package-parent settings-isolation app settings file deleted");
}
...
void appSessionBrokenPackageParentSymlinkLoadFailureLeavesAppSettingsUntouched()
{
    projectname::AppSettings settings;
    settings.audioSetup.firstRunPromptDismissed = true;
    settings.audioSetup.preferredOutput.hasOutputDevice = true;
    settings.audioSetup.preferredOutput.deviceType = "Windows Audio";
    settings.audioSetup.preferredOutput.deviceName = "Session Broken Parent Isolation Output";
    settings.audioSetup.preferredOutput.sampleRateHz = 48000.0;
    settings.audioSetup.preferredOutput.bufferSizeSamples = 256;
    settings.audioSetup.preferredOutput.outputChannelCount = 2;
    settings.audioSetup.preferredOutput.juceDeviceStateXml =
        "<DEVICESETUP deviceType=\"Windows Audio\" audioOutputDeviceName=\"Session Broken Parent Isolation Output\"/>";

    const auto settingsPath =
        makeTemporarySettingsPath("projectname-session-broken-package-parent-load-settings-isolation-test");

    std::string error;
    expect(projectname::saveAppSettings(settings, settingsPath, error),
           "Session broken package-parent settings-isolation fixture saves app settings");
    const auto originalSettingsText = readTextFile(settingsPath);

    projectname::AppSession session;
    session.getProject().setName("Keep Session Broken Parent Settings Isolation Baseline");
    session.setTempoBpm(139.0);
    const auto originalProject = session.getProject();

    const auto parentSymlink =
        makeTemporaryPackagePath("projectname-session-broken-package-parent-load-settings-isolation-link-test");
    const auto missingParentTarget =
        makeTemporaryPackagePath("projectname-session-broken-package-parent-load-settings-isolation-target-test");
    const auto requestedPackage = parentSymlink / "Nested Session Load.project";
    ...
    error = "stale session broken package-parent settings-isolation load error";
    expect(!session.loadProjectPackage(requestedPackage, error),
           "Session load rejects a broken intermediate parent during settings-isolation check");
    expect(error.find("Project package path contains a symlink") != std::string::npos
               && error.find(parentSymlink.generic_string()) != std::string::npos,
           "Session broken package-parent settings-isolation load failure error is human-readable");
    expect(error.find("not found") == std::string::npos,
           "Session broken package-parent settings-isolation load failure is not reported as a missing manifest");
    expect(error.find("JSON") == std::string::npos,
           "Session broken package-parent settings-isolation load failure does not parse JSON through the symlink");
    expect(session.getProject() == originalProject,
           "Session broken package-parent settings-isolation load failure keeps current session project");
    expect(readTextFile(settingsPath) == originalSettingsText,
           "Session broken package-parent settings-isolation load failure leaves app settings unchanged");

    std::string settingsError;
    const auto reloadedSettings = projectname::loadAppSettings(settingsPath, settingsError);
    expect(reloadedSettings.has_value() && *reloadedSettings == settings,
           "Session broken package-parent settings-isolation load failure leaves app settings loadable");
    expect(std::filesystem::is_symlink(std::filesystem::symlink_status(parentSymlink)),
           "Session broken package-parent settings-isolation load failure leaves the parent symlink unchanged");
    expect(!std::filesystem::exists(missingParentTarget),
           "Session broken package-parent settings-isolation load failure does not create the missing parent target");
    expect(!std::filesystem::exists(missingParentTarget / "Nested Session Load.project"),
           "Session broken package-parent settings-isolation load failure does not create a package through the broken link");
    expect(!std::filesystem::exists(requestedPackage / "manifest.json.tmp"),
           "Session broken package-parent settings-isolation load failure does not create a temporary manifest through the link");
    expect(!std::filesystem::exists(missingParentTarget / "Nested Session Load.project" / "manifest.json.tmp"),
           "Session broken package-parent settings-isolation load failure does not write target temporary manifest");

    expect(std::filesystem::remove(parentSymlink),
           "Temporary session broken package-parent settings-isolation symlink deleted");
    std::filesystem::remove_all(missingParentTarget);
    expect(std::filesystem::remove(settingsPath),
           "Temporary session broken package-parent settings-isolation app settings file deleted");
}
...
int main()
{
    ...
    appSessionInvalidDevicesSchemaLoadFailureLeavesAppSettingsUntouched();
    ...
    appSessionLinkedPackageParentSymlinkLoadFailureLeavesAppSettingsUntouched();
    appSessionBrokenPackageParentSymlinkLoadFailureKeepsSessionProject();
    appSessionBrokenPackageParentSymlinkLoadFailureLeavesAppSettingsUntouched();
